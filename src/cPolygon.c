/*      $Id: cPolygon.c 72 2010-05-07 10:29:54Z jraedler $   */
...
staticforward PyMethodDef     Polygon_Methods[27];
...
:Returns:\n\
    numpy bool array\n"
static PyObject *Polygon_isInsideV(Polygon *self, PyObject *args) {
    int i=INDEF, k;
    PyArrayObject *x=NULL, *y=NULL, *inarr=NULL;
    char *in;
    double *ptx, *pty;
    npy_intp npts;
    PyObject *err=NULL;
    if (! PyArg_ParseTuple(args, "OO|i", &x, &y, &i))
        return Polygon_Raise(ERR_ARG);

    if ((x = (PyArrayObject*)PyArray_ContiguousFromObject((PyObject*)x, PyArray_DOUBLE, 1, 1)) == NULL)    return Polygon_Raise(ERR_ARG);
    if ((y = (PyArrayObject*)PyArray_ContiguousFromObject((PyObject*)y, PyArray_DOUBLE, 1, 1)) == NULL)    return Polygon_Raise(ERR_ARG);
    npts = x->dimensions[0];
    if(y->dimensions[0] != npts) return Polygon_Raise(ERR_ARG);

    inarr = (PyArrayObject*)PyArray_SimpleNew(1, &npts, NPY_BOOL);
    in    = (char*)inarr->data;
    ptx   = (double*)x->data;
    pty   = (double*)y->data;
    ...
    if (i!=INDEF) {
        if ((i >= 0) && (i < self->gpc_p->num_contours)) {
            for(k = 0; k != npts; k++) {
                if ((in[k] = poly_c_point_inside(self->gpc_p->contour+i, ptx[k], pty[k])) == -1)
                {
                    err = Polygon_Raise(ERR_INV);
                    break;
                }
            }
        } else
            err = Polygon_Raise(ERR_IND);
    } else {
        for(k = 0; k != npts; k++) {
            if ((in[k] = poly_p_point_inside(self->gpc_p, ptx[k], pty[k])) == -1)
            {
                err = Polygon_Raise(ERR_INV);
                break;
            }
...
static PyMethodDef Polygon_Methods[] = {
    ...
    {"isInsideV",     (PyCFunction)Polygon_isInsideV,   METH_VARARGS, DOCSTR_POLY_ISINSIDEV},
...
int Polygon_cmp(Polygon *o1, Polygon *o2)
{
    if (! Polygon_Check(o1)) return 1;
    if (! Polygon_Check(o2)) return -1;

    gpc_polygon *p1 = o1->gpc_p, *p2 = o2->gpc_p;
    if(p1->num_contours != p2->num_contours)
    {
      return p1->num_contours < p2->num_contours ? -1 : 1;
    }
    
    int i;
    for(i=0; i<p1->num_contours; i++)
    {
      // both holes
      int h1 = *((p1->hole)+i), h2 = *((p2->hole)+i);
      if(h1 != h2)
      {
        return h1 == 1 ? -1 : 1;
      }
      ...
      gpc_vertex_list *c1 = p1->contour + i, *c2 = p2->contour + i;
      if(c1->num_vertices != c2->num_vertices)
      {
        return c1->num_vertices < c2->num_vertices ? -1 : 1;
      }

      // byte-wise compare vertices
      int cmp = memcmp(c1->vertex, c2->vertex, c1->num_vertices*sizeof(*c1->vertex));
      if(cmp != 0)
      {
        return cmp;
      }
...
long Polygon_hash(Polygon *o1)
{
    if (! Polygon_Check(o1)) return -1;

    double hash = 0.;

    gpc_polygon *p1 = o1->gpc_p;
    int i, j;
    for(i=0; i<p1->num_contours; i++)
    {
      gpc_vertex_list *c1 = p1->contour + i;

      hash += *((p1->hole)+i);
      hash += c1->num_vertices;

      for(j = 0; j != c1->num_vertices; j++)
      {
        hash += c1->vertex[j].x;
        hash += c1->vertex[j].y;
      }
